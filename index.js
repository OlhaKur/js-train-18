...
function checkData(data) {
  try {
    ...
    throw new Error("Об'єкт пустий" + error.massage);
  }
}
...
function parseJson(jsonStr) {
  try {
    ...
    // throw new Error("Помилка аналізу JSON: " + error.message);
    console.log(error);
  }
...
function getAge(age) {
  try {
    ...
    const AgeError = new Error(` Вік не може бути менше 0!`, { cause: error });
    console.log(AgeError.name);
  }
...
function decodeURIComponentWrapper(encodedString) {
  try {
    ...
    console.error("Помилка URI: ", error.message);
  }
...
function evaluateExpression(expression) {
  try {
    ...
    console.error("Помилка оцінки виразу:", error.message);
  }
